 ...
 * 集計は質問者の所属部署ごと（複数部署所属の人は各部署に数える）。
 */
import { mkdir, writeFile } from "node:fs/promises";
import { departmentLabel, type DepartmentId } from "../config.js";
import { ingestSql } from "../db/client.js";
import { sendDm, slackWebClient } from "../slack/client.js";
import { flag, runCli, value } from "../lib/cli.js";

function monthRange(ym?: string): { label: string; from: Date; to: Date } {
  const now = new Date();
  // 既定は先月。ym は "YYYY-MM"
  const [y, m0] = ym ? [Number(ym.slice(0, 4)), Number(ym.slice(5, 7)) - 1] : [now.getUTCFullYear(), now.getUTCMonth() - 1];
  const from = new Date(Date.UTC(y, m0, 1));
  const to = new Date(Date.UTC(y, m0 + 1, 1));
  return { label: from.toISOString().slice(0, 7), from, to };
}
...
runCli(async () => {
  ...
  const md = [
    ...
    "## 取り消し（redact）の記録",
    "",
    redactions.length === 0 ? "なし" : redactions.map((x) => `- ${x.created_at.toISOString().slice(0, 10)} 「${x.title}」 理由: ${x.reason}（影響ログ ${x.affected_logs} 件）`).join("\n"),
  ].join("\n");
  ...
  for (const m of managers) {
    ...
    const text = [
      ...
      .join("\n\n");
    try {
      if (await sendDm(slack, m.slack_user_id, { text })) sent++;
    } catch (e) {
