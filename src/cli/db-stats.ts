...
async function main() {
  const sql = ingestSql();
  const [db] = await sql`select pg_size_pretty(pg_database_size(current_database())) as size,
                                 pg_database_size(current_database())::bigint as bytes`;
  const tables = await sql<{ name: string; size: string; rows: number }[]>`
    select relname as name, pg_size_pretty(pg_total_relation_size(relid)) as size, n_live_tup::int as rows
    from pg_stat_user_tables order by pg_total_relation_size(relid) desc`;
  const [docs] = await sql`select count(*)::int as n from documents`;
  const [chunks] = await sql`select count(*)::int as n, coalesce(sum(token_count),0)::int as tokens from chunks`;
  const [byDept] = await sql`select json_object_agg(department_id, n) as j from (select department_id, count(*)::int as n from documents group by 1) t`;
