...
## 14. 実装後の更新（2026-09-06、MVP 完了時点）

計画どおりに完了した項目と、実装して分かって変えた点。詳細は README の「計画からの主な変更点」と各 docs を参照。

### 完了状況
- 15 営業日計画のうち Day 1〜14 の成果物をすべて実装し、納品物チェックリスト 8 項目を手動で確認済み（README の表）。
- 文書 36 件（+ 検証用 1 件）、評価 15 問で Recall@5 1.00、該当なし 3/3、RLS 1/1（docs/eval-latest.md）。
- GitHub: https://github.com/atsusagi111-dot/knowledge-rag-slack（公開。秘密情報なし）。

### 計画から変えた点
| 項目 | 計画 | 実際 | 理由 |
|---|---|---|---|
| 生成モデル | gpt-5-nano | gpt-5-mini（reasoning_effort=low） | nano は該当文書が無い場面で別案件の数値を流用することがあった。mini は安定し 3〜7 秒 |
| チャンク | 見出し単位 | 見出し単位 + 先頭に「文書タイトル > 見出し」 | 客先名入りの質問で数値だけのセクションが拾えなかった |
| topK | 5 | 8 | 36 件で同一文書の別セクションが漏れた |
| 文書の置き場 | git 管理外 | `data/docs/` を git に含める | GitHub Actions が取り込むため。架空文書のみ。実案件では除外に戻す |
| Slack の回答先 | スレッド | チャンネルでの質問は本人の DM | チャンネルの他メンバーに回答を見せない |
| 定期実行 | 3 日おき | 毎日 02:00 取り込み / 週次リトライ / 月次レポート | クライアント向け運用手順書に合わせた |
| ガバナンス | 未計画 | 機密区分（7 番目の部署）、部署変更トリガー、`redact`、`--prune`、`--sync`、月次レポート | 「誤アップロードの取り消し」要件への対応 |
| 監査ログの書き漏れ防止 | 要件メモ: 検索と同一トランザクションで書く | 検索 → 生成 → ログ保存の順で別トランザクション。ログ保存に失敗したら回答を返さずエラー | ログには「回答できたか」「所要時間」を入れるため生成後でないと確定しない。生成中（2〜10 秒）に DB 接続を占有しない。「ログの無い回答は存在しない」という狙いは失敗時エラー化で満たす |

### 費用の実績（2026-09-06 の 1 日）
- Embedding: 37 件 × 3 回の再取り込みを含めて 1 円未満
- 生成: 15 問評価 × 6 回 + 手動確認 約 100 問 ≒ 30〜50 円（mini 切替後は 1 問 0.3 円程度）

### Phase 2 への申し送り
- 5,000 件投入は Supabase Pro + Batch API + HNSW インデックス（設計は計画 10 章のまま有効）
- SharePoint 連携は `SharePointSource` の 2 メソッド実装 + `redact` の元ファイル削除を Graph API に置換
- 専用の管理画面を作る場合は、Table Editor で行っている 2 操作（部署変更・削除）と `redact` / `report` の呼び出しを画面化するだけでよい
